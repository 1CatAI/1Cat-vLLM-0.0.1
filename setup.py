...
def _get_torch_cuda_arch_list() -> list[Version] | None:
    arch_list = os.getenv("TORCH_CUDA_ARCH_LIST")
    if not arch_list:
        return None

    arch_versions = []
    for entry in re.split(r"[;, ]+", arch_list):
        if not entry:
            continue
        arch = entry.removesuffix("+PTX").strip()
        match = re.search(r"(\d+)(?:\.(\d+))?", arch)
        if match is None:
            continue
        major = match.group(1)
        minor = match.group(2) or "0"
        arch_versions.append(Version(f"{major}.{minor}"))

    return arch_versions or None


def _targets_cuda_arch_at_least(min_arch: str) -> bool:
    target_arches = _get_torch_cuda_arch_list()
...
if _is_cuda():
    if _targets_cuda_arch_at_least("8.0"):
        ...
        logger.info(
            "Skipping vllm-flash-attn extensions because "
            "TORCH_CUDA_ARCH_LIST=%s only targets pre-Ampere GPUs.",
            os.getenv("TORCH_CUDA_ARCH_LIST"),
        )
    if _targets_cuda_arch_at_least("9.0") and (
